...
class Expert:
    ...
    def to_api(self) -> dict:
        """API shape(camelCase,与桌面端 ExpertItem 对齐)。"""
        d = self.to_dict()
        return {
    ...
    @classmethod
    def from_dict(cls, d: dict, is_builtin: bool = False) -> "Expert":
        return cls(
            id=str(d["id"]),
            ...
            is_builtin=bool(d.get("is_builtin", is_builtin)),
...
def load_expert_file(path: Path, is_builtin: bool = False) -> Expert | None:
    """读单个专家 JSON;损坏文件返回 None(启动不被一个坏文件拖垮)。"""
    try:
        d = json.loads(path.read_text(encoding="utf-8"))
        return Expert.from_dict(d, is_builtin=is_builtin)
    except Exception:
        return None
