...
**Q1 · SoulBuddy 实现：** SoulBuddy **不做对等多 Agent**，只有「**主 Agent + 按需委托的子代理**」。方案来自 `subagents/` 包：主 Agent 通过 `task` 工具把**自包含子任务**交给 sub-agent（如 `explore`），sub-agent 在隔离上下文里跑自己的循环，返回结构化 JSON。它符合「该上多 Agent 的场景」中的两条：**任务可分解**（探索类子任务独立）、**权限隔离**（子代理只拿 `cfg.tools` 收窄后的工具集，且 ASK 降级为 DENY）。**不是**角色分工/并行/对抗审查式的多 Agent。选型理由是「任务边界清晰、协调成本可能大于收益、延迟敏感」——正是题目里该用单 Agent + 强工具的情形。
...
**Q2 · SoulBuddy 实现：** SoulBuddy 缓解注意力漂移的手段是「**上下文工程**」而非多 Agent：① `context/compact.py` 分层压缩（超长 tool_result 截断、旧文件读 supersede、重复轮去重、L4 摘要 + durable 事实块）；② 检索注入关键句（durable 块常驻 system prompt）；③ **约束前置**（首轮强制推理守卫要求先分类再动手）；④ 把「需要 3–4 次以上工具调用的探索」**委托给子代理**，避免主上下文被文件内容污染（这正是「拆分子目标使每个子上下文更短」）。
...
**Q3 · SoulBuddy 实现：** SoulBuddy 的容错体现在**子代理层**：① sub-agent 异常被 `try/except` 捕获，返回 `{"status": "error", "summary": ...}` 而不让主管道崩；② 失败的委托记 `subagent_failed` 审计；③ sub-agent 有 `max_turns` / `max_time_s` 超时上限，返回 `truncated`；④ 子代理工具 ASK 降级 DENY（不打扰用户）。因为只有「主 + 子」两层、没有多角色对话，所以「互相甩锅」**不涉及**，但「明确终止条件 + 可观测日志」都做到了。
...
**Q4 · SoulBuddy 实现：** SoulBuddy 更接近 **Boss-Worker**（主 Agent 是 Boss，sub-agent 是 Worker），但**没有动态任务图** —— 委托是一次 `task` 调用，一个自包含 prompt，一次同步返回。它也不是 Pipeline（没有固定阶段顺序）。所以准确说是「**简化的 Boss-Worker / 委托式**」。
...
**Q5 · SoulBuddy 实现：** SoulBuddy **没有民主讨论模式**，因此「开不完会」问题**不涉及**。它的「硬终止条件」体现在单 Agent 循环上：`MAX_TURNS=40` + 上下文硬上限 + 重复调用熔断。
...
**Q6 · SoulBuddy 实现：** SoulBuddy **在主/子代理间没有任何异步通信机制** —— 委托是**同步阻塞调用**：主 Agent 调 `task` 工具 → sub-agent 同步跑完 → 返回一份 JSON 摘要 → 主 Agent 继续。不存在黑板、Pub-Sub 或消息队列。它唯一的「共享状态」是 `ToolContext`（传给工具的执行上下文，含 session/workspace/registry 等）。所以「黑板 vs 队列」这套取舍 SoulBuddy **不涉及**。
...
**Q7 · SoulBuddy 实现：** SoulBuddy 的「分配」是**模型自己决定要不要委托（skill-based 的极简形态）**，没有调度器、没有负载均衡、没有竞拍。可控性靠：sub-agent 的 `max_turns` / `max_time_s` 上限、`SUBAGENT_FORBIDDEN_TOOLS` 禁用工具、以及**系统提示词明确约定「3–4 次以上工具调用才委托」**（防止滥委托）。因此「动态任务分配 vs 固定 Pipeline」的取舍在这里简化为「要不要用 `task` 工具」这一个决策。
...
**Q8 · SoulBuddy 实现：** SoulBuddy **没有多 Agent 投票/仲裁**（没有对等意见分歧）。但它的「证据门槛」思想体现在 **rubric**：Gating 硬门槛（G1 安全 / G2 完成 / G3 无残留错误）**只信客观信号、不信模型自述**，且 G1 违反立即终止不重修（INV-19）。这是比投票更硬的「证据门槛 + 优先级规则」。Red Team 专门的对抗 Agent **不涉及**。
...
**Q9 · SoulBuddy 实现：** SoulBuddy 的「状态」不用状态机枚举，而是**显式数据 + 事件流**：会话状态在 SQLite 索引 + JSONL transcript（唯一真相源）；单次 run 的状态是 `_call_counter` / `turns_used` / `rubric.retries_used` / `CompactController.durable_block`；工具调用状态用**事件序列**记录（`function_call`/`function_call_result`/`permission_*`）。这比「自然语言传递一切」更可校验、可回放 —— 与题目结论一致。**乐观锁 / Redis 多进程**因它是单进程单 worker（有硬断言）而**不涉及**。
...
**Q10 · SoulBuddy 实现：** SoulBuddy **既不用 AutoGen 也不用 LangGraph**，是自研的「主循环 + 委托式子代理」。如果硬要类比：它的主循环气质偏「显式状态 + 可恢复 + 可审计」（像 LangGraph 生产取向，但用命令式实现），而子代理偏「隔离上下文的对话式执行」（像 AutoGen 的隔离会话，但更简单、无多轮对话）。「混用框架」**不涉及**。
...
**Q11 · SoulBuddy 实现：** SoulBuddy 不用 CrewAI 的 Crew 抽象，但用「**声明式 sub-agent 配置**」达到了类似目的：`subagents/<name>/agent.yaml` 声明 name/description/tools/model/max_turns 等（三层优先级 builtin < user < project），启动时只扫 frontmatter 构建索引注入 system prompt；主 Agent 需要时通过 `task` 工具实例化。这把「角色 + 工具权限 + 委托契约」从 prompt 里抽成了一等公民 —— 与 Crew 的动机相同，且是声明式、可版本化。
...
**Q12 · SoulBuddy 实现：** 框架选型问题**不涉及**（自研）。但 SoulBuddy 的生产取向值得一提：它把「强测试（162 用例，rubric 用 offline provider 零 token 跑）、强权限（顶层 permissions 包）、强监控（事件流 + 审计链 + LangSmith）、成本与延迟控制（MAX_TURNS / 预算告警 / 压缩）」全部自己做在了主循环里 —— 这正是「框架本身不替你完成的那些」。

### 7.4 · SoulBuddy 实现（框架对照）

SoulBuddy **不使用下列任何多 Agent 框架**：AutoGen / CrewAI / MetaGPT / ChatDev / LangGraph。它的对照关系是：**声明式 sub-agent（`agent.yaml`）+ 同步委托（`task` 工具）+ 隔离上下文（独立 messages/registry）**，只借用了「角色与任务分离」的思想，没引入框架的对话编排或图执行。
...
**Q13 · SoulBuddy 实现：** SoulBuddy **不涉及 BPM 工作流引擎**。它的「确定性 vs 推理」边界体现为：**权限判定、路径守卫、压缩管线、rubric Gating** 是确定性的规则代码；**任务分解、工具选择、最终答复** 由 LLM 推理。即「Agent 主、护栏是规则代码」。
...
**Q14 · SoulBuddy 实现：** SoulBuddy 用 (1)+(4)：**全局步数上限**（`MAX_TURNS=40`，子代理另有自己的上限）+ **预算熔断**（同 `(tool, args)` 重复 ≥3 次熔断、上下文硬上限受控终止）。**状态哈希去重**（连续重复同一计划）与**无进展检测**（指标多轮不变）SoulBuddy **不涉及**。误杀处理：达上限**保留副作用、不自动回滚**，用户可 `rollback_session`。
...
**Q15 · SoulBuddy 实现：** 用 (1)+(2)+(3)+(4) 的组合：**收窄工具集**（sub-agent 只保留 `cfg.tools` 声明的工具，且强制剔除 `SUBAGENT_FORBIDDEN_TOOLS`）、**校验 Agent / 门禁**（`permissions.decide` 在 sub-agent 内也走一遍，ASK 直接降级 DENY）、**不把未校验的自然语言当 API 参数**（工具参数经 provider 原生 tool-calling 结构化）。**项目内不涉及 BPM 式的审批工作流**。工具返回很大时对应做法是 Externalizer 外置 + L1 截断（同前文 Q6）。
...
**Q16 · SoulBuddy 实现：** SoulBuddy 只有「主 + 子代理」，不存在多 Agent 之间的接口契约不一致问题。唯一的「契约」是 sub-agent 的**返回 JSON schema**（`status/summary/artifacts/findings/next_steps`），由 `_parse_result` 做**降级填充**（缺字段补默认、status 落在合法集），保证主 Agent 永远拿到合法 JSON —— 这就是它的「契约强制」。

**Q17：如何做跨 Agent 的权限隔离？**  
**A：**工具 **分账户/分密钥**；Agent **最小权限**；敏感操作走 **审批工作流**；审计日志 **不可篡改存储**。

**Q17 · SoulBuddy 实现：** SoulBuddy 的子代理权限隔离是**三者都有**：① **最小权限**（sub-agent 用**收窄的 ToolRegistry**，只含 `cfg.tools` 声明的工具）；② **审批降级**（ASK 在 sub-agent 层直接转 DENY，不打扰用户，落 `subagent_ask_downgraded` 审计）；③ **审计**（子代理事件写 `subagent_*` 审计，主上下文只看一条 tool_result）。「分账户/分密钥」在**主/子代理之间不涉及**（同一进程同一凭据），MCP 层才有凭据隔离（env 注入子进程）。

**Q18：多 Agent 的评估怎么做？**  
**A：**分层：**单元**（单 Agent I/O）、**集成**（两两交互）、**端到端**（任务成功率）；辅以 **LLM-as-judge** 需防偏，最好配 **黄金集与人审**。

**Q18 · SoulBuddy 实现：** SoulBuddy 的评估**不是多 Agent 分层**，而是「单 Agent 的 rubric 分层」：规则层（Gating/Q1–Q4）+ LLM judge（Q5/Q6）+ 端到端 smoke（`script/rubric_smoke.py` 跑真实 `SoulAgent.run()`）。子代理只被间接评估（它的返回值是否帮主 Agent 完成任务）。专门的「黄金集与人审」流程**不涉及**。

**Q19：为什么需要「人机在环」？**  
**A：**高风险决策、未知法规、或 **模型置信度低** 时，人类是 **最后防线**；同时可 **收集真实反馈** 迭代提示与工具。

**Q19 · SoulBuddy 实现：** SoulBuddy 的 HITL 就是 **PermissionGate**：bash 与 MCP 调用（高风险）走 ASK，用户可「本次允许 / 始终允许（目录级记忆）/ 拒绝 / 拒绝本次 run 后续」，300s 超时转 DENY 且绝不事后补执行。这是「高风险时人类是最后防线」的直接落地（见 `docs/modules/17-human-in-the-loop.md`）。

**Q20：多 Agent 与「单 Agent + 多个工具」取舍？**  
**A：**若只需 **统一策略** 调不同 API，单 Agent + 工具即可；若需要 **角色隔离、并行、对抗评审、组织流程**，多 Agent 更合适。

**Q20 · SoulBuddy 实现：** SoulBuddy 明确选了「**单 Agent + 多工具**」为主（15 内置 + 动态 MCP），只在「**上下文隔离**」这一条诉求上引入了子代理（探索类任务不污染主上下文）。它**不要**并行、对抗评审、组织流程 —— 因为面向本地单用户编码场景，协调成本大于收益。
