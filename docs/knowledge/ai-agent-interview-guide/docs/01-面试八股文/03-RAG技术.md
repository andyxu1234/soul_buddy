...
**面试 Q1 · SoulBuddy 实现：** SoulBuddy 的资料库 RAG 就是这样：用户上传 md/txt/pdf/docx → 后台索引进 Milvus → 对话中模型调用 `search_knowledge(query, top_k)` → 返回带出处的片段 → 模型据此回答并引用。检索是一个**普通工具**（agentic 检索），不是每轮全量注入。

**追问应对**  
- **问：RAG 和「把文档全文塞进 Prompt」有什么区别？**  
  **答：** 全文往往超长、噪声大、成本高；RAG 通过检索只取 **最相关的一小部分**，在 **效果、延迟、费用** 上更可控，也更适合大规模知识库。

**追问 · SoulBuddy 实现：** SoulBuddy 明确走 RAG 而非全文注入：文档先落盘到 `~/.soul_buddy/kb/uploads/`、切块（`KB_CHUNK_TOKENS=700`、15% 重叠）、向量化入 Milvus，检索默认只取 `KB_TOP_K=5`（单次上限 10）条。这既控制了 token，也避免了长文档噪声。
...
**面试 Q2 · SoulBuddy 实现：** SoulBuddy 的资料库数据完全本地化（`~/.soul_buddy/kb/`，含 `kb.db` 元数据 + `milvus.db` 向量 + `uploads/` 原文，整个目录可整体备份/删除），符合「私域数据不出域、可审计、可更新」的思路；文档更新走 reindex（`ready → pending`）而非重训模型。

**追问应对**  
- **问：RAG 能完全消除幻觉吗？**  
  **答：** 不能。若检索错、块质量差、或模型忽略证据，仍可能幻觉。需要检索、重排、引用约束、评估与人工审核配合。

**追问 · SoulBuddy 实现：** SoulBuddy 也没号称消除幻觉，而是用工程手段降低：返回「文档名 > 章节 > 相关度」出处供引用、空结果时工具明确回「资料库中没有找到相关内容」并要求模型声明「这不在你的资料库范围内」；此外还支持 milvus-lite 不支持 BM25 时**自动降级纯 dense 检索**并打 `hybrid_fallback` 标记（避免检索直接失败）。
...
**面试 Q3 · SoulBuddy 实现：** SoulBuddy **不涉及微调**，全部靠「Prompt + 工具契约 + 原生 tool-calling」来固定行为与格式（不训练权重）。事实类知识则完全走 RAG（资料库）和三层记忆。
...
**面试 Q4 · SoulBuddy 实现：** SoulBuddy 两条管线**只在 Milvus 处相遇**：
- **离线（索引管线，上传时异步）**：解析 → 分块 → embedding → 写入 Milvus；上传立即返回，状态机 `pending → parsing → chunking → embedding → indexing → ready/failed` 实时落 `kb.db`，桌面端每 2s 轮询刷新；启动时 `enqueue_pending()` 把卡在中间状态的文档重新入队（重启自愈）。
- **在线（查询管线，对话时实时）**：模型调 `search_knowledge` → 查询向量化 → 混合检索（dense + BM25 → RRF）→ 拼出处 → 模型带引用回答。
...
**面试 Q5 · SoulBuddy 实现：** SoulBuddy 的 PDF 解析用的是 **`pypdf` 逐页抽文本**（`md/txt` 直读、`utf-8` 失败回退 `gb18030`；`docx` 用 `python-docx` 抽段落 + 表格行）。它**不做 OCR**（见 Q6），所以扫描版 PDF 抽不出文本时会直接进 `failed` 状态并落错误信息，UI 可一键重新索引。这正印证了 Q5 的坑①：无文本层必须 OCR，而 SoulBuddy 对此的处理是「明确失败 + 可操作提示」而非静默出错。

**追问应对**  
- **问：为什么很多团队推荐 Unstructured？**  
  **答：** 它把文档分成带类型的元素（Title、NarrativeText、Table 等），便于 **按结构分块** 和后续路由（表格走表格策略）。

**追问 · SoulBuddy 实现：** SoulBuddy **不用 Unstructured**，而是自研了一套「**标题感知**」的分块：按 ATX 标题（`#`/`##`）切 section，用**标题栈**维护 `heading_path`（`文档名 > 章节 > 小节`），段内再按空行分段贪心打包到 ~700 token、携带 15% 尾部段落重叠，超大段落自适应字符硬切、碎片向后合并。这达成了 Unstructured「按结构分块」同样的目的，且 `heading_path` 直接用作检索结果的出处定位。
...
**面试 Q6 · SoulBuddy 实现：** SoulBuddy **不涉及 OCR**（不集成 Tesseract / PaddleOCR 等），扫描版 PDF 会因抽不出文本而 failed，不会带着错字入库。但它间接印证了「需要混合检索提高鲁棒性」这一点：SoulBuddy 正是用 **dense + BM25 稀疏 → RRF 融合** 的混合检索，即使文本有少量不规范（例如 markdown 里的符号、编号），BM25 仍能抓到精确关键词命中。
...
**面试 Q7 · SoulBuddy 实现：** SoulBuddy 只做了**最基础**的处理：`docx` 解析会把表格行抽成文本（`python-docx` 的段落 + 表格行），PDF 走正文抽文本。它**没有专门的表头感知分块、表格转 Markdown/JSON、图片 Caption 或结构化检索**，也就是说「表格/图片的最佳实践」在 SoulBuddy 里**大部分不涉及** —— 复杂表格可能被切成碎片，属于已知短板。
...
**2.4 · SoulBuddy 实现：** SoulBuddy **不涉及 Tika**（纯 Python：`pypdf` + `python-docx` + 直读）。文本标准化方面做了编码兜底（`utf-8` → `gb18030`）和按空行/标题的结构化切分，但没有独立的「批量清洗/去重（MinHash/SimHash）/PII 脱敏」流水线。上传校验只有「扩展名白名单 + 文件大小 ≤30MB + 非空」，超出即 rejected 并在 UI 提示跳过原因。
...
**面试 Q8 · SoulBuddy 实现：** SoulBuddy 用 `KB_CHUNK_TOKENS=700`（约 700 token/chunk）+ 15% 尾部段落重叠：既保证 chunk 能覆盖「完整命题」，又通过重叠避免要件被切在边界；同时**不按固定长度硬切**，而是先按标题切 section、再段内按空行贪心打包，所以极少出现「切断半句话」的情况。碎片会自动向后合并。
...
**面试 Q8a · SoulBuddy 实现：** SoulBuddy **不采用父子块**结构（不做 parent-child 索引），而是**扁平 chunk + 结构化元数据**：每个 chunk 在 Milvus 里存 `chunk_id`（主键）、`doc_id`（指回 kb.db）、`chunk_index`、`heading_path`、`text`、`sparse`（BM25 自动生成）、`dense`。检索命中后直接用该 chunk 的 `text`，并靠 `heading_path` 定位来源。所以「父子块如何建索引」这个具体机制 SoulBuddy 不涉及。
...
**3.8 · SoulBuddy 实现：** SoulBuddy **不用 LangChain 的 TextSplitter 系列**，而是自研分块器，思路与 `RecursiveCharacterTextSplitter` + `MarkdownHeaderTextSplitter` 类似但更专用：**先按 ATX 标题切 section（用标题栈记录 `heading_path`）→ 段内按空行分段贪心打包到 ~700 token → 加 15% 尾部段落重叠 → 超大段落自适应字符硬切、碎片向后合并**。既保留了 Recursive 的「优先在自然边界断开」，又加上了 Markdown 标题感知与出处路径记录。
...
**面试 Q9 · SoulBuddy 实现：** SoulBuddy 用的是 **OpenAI 兼容的 `/embeddings`**（`EMBEDDING_BASE_URL` / `EMBEDDING_API_KEY` / `EMBEDDING_MODEL` 可配），走的是通用多语言模型，所以默认对中英混合友好，且不额外做「按语言分索引」。它还有一个很关键的维度保护：**首次索引探测向量维度并缓存，后续与已索引维度比对，不一致（即换了 embedding 模型）直接将该文档标 failed 并提示删除后重新上传** —— 因为不同模型的向量空间不可混比。混合检索（BM25）也补齐了关键词/术语/编号的精确命中。
...
**面试 Q10 · SoulBuddy 实现：** SoulBuddy 选了 **Milvus**（`pymilvus`），默认用 **`milvus-lite` 本地文件**（`MILVUS_URI` 指向 `milvus.db`），填 `http://localhost:19530` 即可切 standalone，**代码零改动**。选 Milvus 而非 FAISS 的原因正是本问题答案：Milvus 提供了每库一个 collection（`kb_<kb_id>`）的数据管理、`doc_id` 过滤、`sparse`+`dense` 双字段（支持 hybrid_search），省去了自建存储与服务。它用 `replace_doc`（同文档先删后插）保证幂等。
...
**面试 Q5a · SoulBuddy 实现：** SoulBuddy 的 dense 索引用的是 **FLAT + IP（内积=余弦）** —— 即**精确检索（暴力）**，不是 HNSW/IVF。这个选择与它的定位一致：面向**个人本地知识库**（数据量小、chunk 数有限），精确检索足够快且召回 100%，避免了 ANN 的调参（efSearch 等）与精度损失。BM25 稀疏向量则由 Milvus 内置 BM25 Function 自动生成。也就是说「HNSW vs IVF 取舍」在这个规模下**不需要考虑**。

**追问应对**  
- **问：efSearch 调大有什么效果？**  
  **答：** HNSW 查询时访问更多邻居，**召回上升、延迟上升**；需在精度与 SLA 间折中。

**追问 · SoulBuddy 实现：** SoulBuddy **不涉及 HNSW/efSearch**（用 FLAT 精确检索）。它的检索质量调节点是 `KB_TOP_K`（默认 5，单次上限 10）与 `RRFRanker(60)` 融合的 k 值，而非 ANN 参数。
...
**面试 Q11 · SoulBuddy 实现：** 这是一个**直接命中**的题 —— SoulBuddy 的资料库检索就是「dense 向量（抓语义，换说法也能中）+ BM25 稀疏（抓关键词/术语/函数名/编号）→ RRF 融合」。因为资料库主要是技术文档（markdown 笔记等），「同一概念不同表述」和「精确术语/编号」两类需求都存在，所以混合检索是刚需。它还处理了降级：`milvus-lite` 不支持 BM25 时自动降级纯 dense 并打 `hybrid_fallback` 标记。

**面试 Q11a：RRF 与加权分数融合各适合什么？**  
**标准答案 A：** 当两路分数 **量纲一致或已可靠归一化** 时，加权融合直观可调；当一路是 **排名**、一路是 **概率**、或分数 **不可比** 时，**RRF 更稳**，且少调参（经典 \(k=60\)）。

**面试 Q11a · SoulBuddy 实现：** SoulBuddy 用的正是 **RRF**（`RRFRanker(60)`），而不是加权分数融合。原因是 dense 的 IP 分数和 BM25 的稀疏分数**量纲不可比**（正是本问题所说的情况），RRF 只用排名融合、少调参、更稳，非常适合「轻量本地库」这种不想引入调参负担的场景。
...
**面试 Q11b · SoulBuddy 实现：** SoulBuddy **不做查询改写**（不涉及 HyDE / 子问题分解 / Step-back），`search_knowledge` 直接用模型传入的 query 检索。因此「改写引入噪声」这个问题它不涉及 —— 它的策略是「把 query 构造交给模型自己（agentic）」，模型可以在多轮里自己换关键词重查。它用了 RRF 融合来吸收多路的不可比分数。

**追问应对**  
- **问：需要保留原 Query 吗？**  
  **答：** 需要。常 **原句 + 改写句** 同时检索再融合，避免改写跑偏。

**追问 · SoulBuddy 实现：** SoulBuddy **不涉及查询改写**，所以不存在「原句 vs 改写句」的取舍；一次 `search_knowledge` 只发一个 query（由模型决定怎么写）。
...
**面试 Q11c · SoulBuddy 实现：** SoulBuddy **不做显式的子问题分解**（不在检索前把问题拆成子问题序列），所以这些失败模式它不涉及。它走的是「**Agentic 多步检索**」路线：由模型在循环里决定要不要再查、换什么 query，多次调用 `search_knowledge` 直到够用。

**追问应对**  
- **问：和 Agent 多步检索有什么区别？**  
  **答：** 子问题分解可以是 **固定模板/一次 LLM 调用**；Agentic 更强调 **动态决定是否再检索**，工具边界更宽。

**追问 · SoulBuddy 实现：** SoulBuddy 明确站在 **Agentic** 一侧 —— `search_knowledge` 就是注册在工具表里的一个普通工具，模型在多轮循环里自主决定何时查、查几次；查不到可自行换 query。没有「一次性固定模板分解」这一步。
...
**面试 Q12 · SoulBuddy 实现：** SoulBuddy **不使用 HyDE**。它靠「dense（语义近似）+ BM25（精确词）」混合来覆盖「表述差距」与「精确匹配」两侧，不引入「假想答案文档」带来的虚构风险。
...
**面试 Q12a · SoulBuddy 实现：** SoulBuddy **两者都不涉及**。它不做 HyDE、也不做 Step-back，检索 query 就是模型直接给出的那一个。
...
**面试 Q13 · SoulBuddy 实现：** SoulBuddy **不做独立的 Cross-Encoder 重排**。它把「排序」交给 **RRF 融合**（dense + BM25 排名融合）在 Milvus 内完成，然后 `KB_TOP_K=5`（单次上限 10）直接进上下文。也就是说它没有「召回几十到几百再精排」的两阶段，而是「混合检索 + RRF 融合 + 小 top_k」一步到位 —— 对轻量本地库规模够稳，也不需要额外的重排模型与延迟。
...
**面试 Q14 · SoulBuddy 实现：** SoulBuddy **没有 GraphRAG**（不建知识图谱）。它用的是「向量 + BM25 混合的扁平 chunk 检索」，不涉及实体抽取、子图遍历、社区摘要。
...
**面试 Q14a · SoulBuddy 实现：** SoulBuddy 属于 **Agentic RAG**：检索是普通工具（`search_knowledge`），由模型在多轮循环里自主决定何时查、查几次、要不要换 query。它**不是 Self-RAG**（不涉及反思 token / 训练与解码策略），但它的 P6 rubric 里有一层「客观信号验收」（Gating 规则 + LLM judge），算是把「证据是否支持」落到运行时评估，而非解码时内嵌标签。

**面试 Q14b：Corrective RAG 典型触发信号？**  
**标准答案 A：** **检索置信度低**（Top1 与 Top2 差距小）、**检索结果与问题实体不一致**、**重排后仍低分**、或 **生成与引用冲突**（可做一致性检测）。触发后执行 **查询改写、换索引源、或联网搜索**。

**面试 Q14b · SoulBuddy 实现：** SoulBuddy **没有自动的 Corrective RAG 触发逻辑**（不检测检索置信度、不自动改查或换源）。它的「纠正」体现在两点：① 空结果时工具明确回「资料库中没有找到相关内容」，提示词要求模型**声明「这不在你的资料库范围内」**（相当于拒答/降级）；② 因为检索是 agentic 的，模型可以**自己**决定换个 query 再查。

**面试 Q14c：Adaptive RAG 如何避免路由误判？**  
**标准答案 A：** **混合路由**（并行短路与完整链路）、**置信度阈值 + 默认安全策略**（不确定则走更强检索）、持续用 **线上反馈** 迭代分类器。

**面试 Q14c · SoulBuddy 实现：** SoulBuddy **没有自适应路由分类器**，但有更简单粗暴的「路由」：**按会话是否绑定资料库决定 `search_knowledge` 是否可见**（未绑定 → 工具列表里根本没有这个工具，handler 里还有兜底校验）。这避免了「路由误判」问题本身 —— 不需要判断「要不要检索」，而是由「这个会话有没有库」直接决定，再用 agentic 循环让模型决定「何时查」。

**追问应对**  
- **问：小公司是否值得上 GraphRAG？**  
  **答：** 若数据以 **说明文、FAQ** 为主，**向量 RAG + 混合检索 + 重排** 通常足够；图适合 **关系问题占比高** 且团队有 **图谱与评测** 能力时再投入。

**追问 · SoulBuddy 实现：** SoulBuddy 就是「不上 GraphRAG」的典型 —— 面向个人本地笔记/文档，以说明文和技术资料为主，用 **向量 + BM25 混合检索**（连重排都没加）就已够用；不建图谱、不做实体抽取。
...
**面试 Q15 · SoulBuddy 实现：** SoulBuddy **没有针对 RAG 的专门评估框架（不涉及 RAGAS）**，也不在线评检索质量/忠实度。它的评估重心在**任务交付**（rubric 的 Gating/Q1–Q6），RAG 只在「资料库绑定」这一层做约束。所以「检索 vs 生成的瓶颈定位」在 SoulBuddy 里依靠**人工看 transcript 里的出处与引用**来排查，而非自动化指标。
...
**面试 Q16 · SoulBuddy 实现：** SoulBuddy 面向**单用户本地桌面**，**不涉及多租户**。它的等价隔离是**按知识库分 collection**：每个 kb 一个 `kb_<kb_id>` collection，检索时按会话绑定的 `kb_ids` 逐个库查，且有 `doc_id` 过滤（删除/重索引按它过滤）。所以「租户隔离」在这里体现为「知识库隔离 + 按绑定可见」。
...
### 11.x · SoulBuddy 实现（综合题库 Q1–Q24）

**Q1（两步流水线）：** SoulBuddy 的离线是「解析（pypdf/python-docx/直读）→ 分块（标题感知 ~700 token/15% 重叠）→ embedding（OpenAI 兼容，批 64）→ 写 Milvus（同文档先删后插，幂等）」；在线是「模型调 `search_knowledge` → query 向量化 → hybrid_search（dense+BM25→RRF）→ 拼出处 → 生成」。增量更新按 `doc_id` 先删后插，重索引走 `ready → pending`。

**Q2（RAG 与微调配合）：** SoulBuddy **不涉及微调**，只做 RAG。

**Q3（为什么需要 chunk_overlap）：** SoulBuddy 用 **15% 尾部段落重叠**，防止要件被切在块边界；实现上「按段携带尾部段落」而非字符级重叠。

**Q4（分隔符顺序）：** SoulBuddy 的等价物是「**先按 ATX 标题切 section，再段内按空行分段贪心打包**」—— 优先在更大的语义单元（章节、段落）断开，减少碎片化。

**Q5（语义分块 vs 递归分块）：** SoulBuddy **不用语义分块**（不做 embedding 相似度断点），用结构感知分块，成本低、无阈值敏感问题。

**Q6（父子文档如何存储）：** SoulBuddy **不涉及父子块**；用扁平 chunk + `doc_id`/`heading_path` 元数据。

**Q7（Embedding 是否归一化）：** SoulBuddy 的 dense 索引是 `FLOAT_VECTOR` + **FLAT + IP（内积=余弦）**，依赖 embedding 服务返回的向量；归一化由 embedding 侧/度量方式处理，代码层不额外做归一化。

**Q8（IndexFlatIP vs IndexHNSW）：** SoulBuddy 用 **FLAT（精确）**，不用 HNSW —— 因为面向个人本地小库，规模小、要 100% 召回，精确检索足够。

**Q9（混合检索权重 alpha 怎么定）：** **不需要调 alpha** —— SoulBuddy 用 **RRF（`RRFRanker(60)`）** 只融合排名，天然规避了权重调参。

**Q10（RRF 为什么鲁棒）：** 同答案：SoulBuddy 选 RRF 正是因为它只用排名、规避 dense 与 BM25 分数尺度不可比的问题。

**Q11（HyDE 风险如何缓解）：** SoulBuddy **不用 HyDE**，不涉及该风险。

**Q12（BM25 中文要不要分词）：** SoulBuddy 的 BM25 由 **Milvus 内置 BM25 Function** 在 `text` 字段上自动分词产出稀疏向量（`enable_analyzer`），查询时只给原文；分词策略由 Milvus 处理，应用层不自建分词器。

**Q13（Cross-Encoder 为何不能替代向量索引）：** SoulBuddy **不用 Cross-Encoder**，用混合检索 + RRF。

**Q14（MMR 的 lambda）：** SoulBuddy **不用 MMR**（RRF 融合后直接取 top_k），不涉及该参数。

**Q15（GraphRAG 解决什么痛点）：** SoulBuddy **不建图**，不涉及。

**Q16（Agentic RAG 与一次性 RAG 差异）：** SoulBuddy 属于 **Agentic RAG**：`search_knowledge` 是模型自主调用的工具，可多次调用、自己换 query；且工具仅对绑定了库的会话可见。

**Q17（Self-RAG 核心思想）：** SoulBuddy **不是 Self-RAG**（无反思 token / 训练解码策略）；它把「证据是否支持」交由运行时 rubric 验收而非生成时内嵌标签。

**Q18（Corrective RAG 触发条件）：** SoulBuddy **无自动触发**；靠「空结果 → 声明不在范围」+ 模型自行换 query 来纠正。

**Q19（RAGAS 的局限）：** SoulBuddy **不接入 RAGAS**，不涉及。

**Q20（低成本在线评估）：** SoulBuddy 的在线可观测是**事件流 + 落盘报告**（`context_usage`、`rubric/*.json`、LangSmith trace），RAG 本身没有专门的点赞/纠错弱监督信号，**不涉及**。

**Q21（索引频繁更新如何保持一致性）：** SoulBuddy 用「**同文档先删后插（replace_doc）保证幂等** + 状态机实时落 `kb.db` + 启动 `enqueue_pending()` 自愈」；文档更新走 reindex（`ready → pending`）。多副本/双写切换/灰度**不涉及**（单机 lite）。

**Q22（如何防止 Prompt 注入污染 RAG）：** SoulBuddy 的文档内容作为 **tool_result（数据）** 进入上下文，不提升为指令；其权限与工具调用受顶层 `permissions` 约束（资料库检索 `search_knowledge` 是只读工具，自动放行）。专门的「文档清洗 + 异常指令模式检测」**不涉及**。

**Q23（长上下文模型出现后 RAG 会消失吗）：** SoulBuddy 的答案是「不会」—— 资料库正是为了把私域文档按需检索（只取 top_k 5）而不是全量注入，控制成本与噪声。

**Q24（多模态 RAG 要点）：** SoulBuddy **不支持多模态 RAG**（不做图像/表格编码、跨模态对齐），只处理文本；表格仅以文本行形式被抽取。**不涉及**。
